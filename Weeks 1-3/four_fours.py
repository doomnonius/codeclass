# The four fours challenge!

from math import *

print ("Zero is", 4+4-4-4)
print ('One is', (4/4)+4-4)
print ('Two is', (4/4)+(4/4))
print ('Three is', factorial(4)/(4+sqrt(4)+sqrt(4)))
print ('Four is', .4*(4+4+sqrt(4)))
print ('Five is', (4+4+sqrt(4))/sqrt(4))
print ('Six is', (4+4+sqrt(4))-4)
print ('Seven is', (44/4)-4)
print ('Eight is', 4+4+4-4)
print ('Nine is', 4+4+(4/4))
print ('Ten is', 4+4+(4/sqrt(4)))
print ('Eleven is', 44/sqrt(4)/sqrt(4))
print ('Twelve is', 4+4+sqrt(4)+sqrt(4))
print ('Thirteen is', (factorial(4)+sqrt(4))/(4/sqrt(4)))
print ('Fourteen is', 4+4+4+sqrt(4))
print ('Fifteen is', (44/4)+4)
print ('Sixteen is', (((4**4)/4)/4))
print ('Seventeen is', (4*4)+(4/4))
print ('Eighteen is', (4*4)+(4/sqrt(4)))
print ('Nineteen is', factorial(4)-4-(4/4))
print ('Twenty is', (4+(4/4))*4)
print ('Twenty is also', factorial(4)-4+4-4)
print ('Twenty one is', factorial(4)+(4/4)-4)
print ('Twenty two is', factorial(4)+4-sqrt(4)-4)
print ('Twenty three is', factorial(4)-((sqrt(4)+sqrt(4))/4))
print ('Twenty four is', ((sqrt(4)+sqrt(4)+sqrt(4))*4))
print ('Twenty five is', factorial(4)+(sqrt(4)+sqrt(4))/4)